...
interface StepDefinitionStorePropType {
  stepDefinition:any;
  isLoading:boolean;
...
const formatStepDefinitions = (defenitionsRespons:DefinitionPropType[]) => {
  const result: { source_step: string; params: {} }[] = [];
  defenitionsRespons.forEach((record:any) => {
    const sourceStep = record?.fields?.Step.split('\n');
    if (sourceStep.length > 1) {
      const separatedParams = record?.fields?.Params?.split('\n') ?? [];
      sourceStep.forEach((step:any, index:number) => {
        const params = separatedParams[index]?.split(',') ?? [];
        const refactored:any = stepParamsFormatter(step, params);
        result.push(refactored);
      });
    } else {
      const params = record?.fields?.Params?.split(',') ?? [];
      const refactored:any = stepParamsFormatter(record?.fields?.Step, params);
      result.push(refactored);
    }
  });
  return result;
...
export const getApiStepDefinitions = create<StepDefinitionStorePropType>((set) => ({
  stepDefinition: [],
  isLoading: true,
