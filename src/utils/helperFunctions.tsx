...
import { stepDefinitionPropType } from '../components/step';
...
export function convertJsonToOptions(json:TableJson) {
  if (!json) return null;
  const options:any[] = [];

  if (json.type === 'table') {
    const tableRows = json.content.filter((row:TableJson) => row.type === 'tableRow');

    tableRows.forEach((tableRow:TableJson) => {
...
export function jsonToFeature(jsonData:FeatureProps) {
  try {
    ...
    function createLine(obj?:FeatureProps|ScenarioProps|StepProps, description?:string) {
...
export const constructStepView = (step:ParamsStepPropType) => {
  ...
  const array = splittedStep.map((word) => {
...
function getSourceStepWithParams(givenStep:any, predefinedSteps:any) {
  ...
  predefinedSteps.forEach((step:ParamsStepPropType) => {
    const modifiedStep = step.source_step
      .split(' ')
      .filter((word) => !cucumberExpressions.includes(word))
      .join(' ');
...
export function featureToJSON(featureText:string, predefinedSteps:stepDefinitionPropType) {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('Feature:')) {
        ...
        // Check if the stepName matches any predefined step's source_step
...
export const addBackslashOnBefore = (string:string) => {
  ...
  const finalValue = string
    .replace(/(['"])/g, '\\$1') // It matches any single or double quote character and add before backslashes
    .replace(/\\+/g, '\\') // remove more than one backslashes
    .replace(/^'(?![\\"])(.*)'$/g, '$1') // It matches any character that is not a backslash with a double  or single quote.
    .replace(/\\(?![\\"'])/g, ''); // remove solo backslash only
  return finalValue;
};

export function stepParamsFormatter(Step:StepProps, params:string[]) {  
